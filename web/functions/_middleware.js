...
function meta(url) {
  ...
  if (path.endsWith("/fleet")) {
    ...
    if (brands.length > 1) {
      return {
        // Заголовок тоже был вопросом, а не ответом. Точный вердикт сюда
        // не подставить — middleware не тянет brands.json ради тега, —
        // но обещать конкретику вместо риторики можно.
        title: `${brands.join(" × ")}: чем различаются в данных ГИБДД · ${SITE}`,
        desc: "Смертность, тяжесть, вина и характерные нарушения — по 1,6 млн ДТП.",
